#!/usr/bin/env bash
# 235B stock speculation smoke at 12-slot stream cache.
# CPU-only. llama-cli (spec flags). n=16 like the greedy A/B.
# k=4 is expected to abort or serialize (U(4)~19 > 12 slots; waves want 24).
set -euo pipefail
...
OUT=measurements/spec235b_b512
N=${N:-16}
CTX=${CTX:-512}
THREADS=${THREADS:-16}
...
open("$OUT/config.json","w").write(json.dumps({
  ...
  "note": "Smoke. -b 512 -ub 1 (llama-cli -b 1 hits GGML_ASSERT n_tokens_all<=n_batch). Naive k=2 fits 12 slots ~54%. Not 200-prompt G3.",
}, indent=2) + "\n")
PY
...
run() {
  ...
  grep -E "tok/s|eval time|prompt eval|Generation:|draft acceptance|accepted|tokens per second|load time|n_ctx|Maximum resident|abort|error|Cannot allocate|slots" \
    "$OUT/${name}.stderr" "$OUT/${name}.stdout" 2>/dev/null | tee -a "$OUT/meta.txt" || true
  return 0
}

: > "$OUT/meta.txt"
echo "HEAD=$HEAD n=$N ctx=$CTX bin=$BIN" | tee -a "$OUT/meta.txt"
...
out = pathlib.Path("measurements/spec235b_b512")
...
def aborted(text):
    return bool(re.search(r"abort|Cannot allocate|insufficient memory|unique experts", text, re.I))
...
verdict = {
    ...
    "note": "n=16 smoke at 12s. Not 200-prompt G3. Compare tok/s to greedy 0.38.",
}
...
echo "SPEC235B_DONE $(date -Is)" | tee -a "$OUT/meta.txt"
