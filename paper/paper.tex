...
% Paper B (speculation/engine) is a local draft only --- not for arXiv until
% 30B -v restores alpha and 235B beats F10b -ub 8 parity.
% Paper B is not this paper.pdf.
...
\begin{center}
\fbox{\parbox{0.92\linewidth}{\small\textbf{Scope.} This manuscript is the
measurement paper: traces, $U(k)$, mmap/\texttt{O\_DIRECT}, greedy
0.29/0.38\,\tps{}.
Paper~B is a speculation/engine \emph{draft}. It is \textbf{not} for arXiv
until a 30B \texttt{-v} smoke restores $\alpha$ and 235B 12-slot decode
beats F10b \texttt{-ub 8} parity. Paper~B is \textbf{not} this repository's
PDF (\url{https://github.com/Shriniwas410/capacity-law-serving/blob/main/paper/paper.pdf}).}}
\end{center}
...
0.44\,\tps{} GPU-split figure is excluded. Speculative-decode wall-clock
and admission-engine experiments are not claims of this paper (see the
scope box).
