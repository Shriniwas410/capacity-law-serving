...
% Capacity-law serving (Paper 2). Compile: tectonic paper.tex
% Companion training paper: Cacheable by Design (arXiv 2608.18261), cited as suram2026cacheable.
...
\large Routing Traces, Union Demand, and Why Stock Speculation Loses on a Consumer SSD}
...
admission cap has mean admitted length $T{=}1.003$. A 0.6B same-family draft is
accepted at $\alpha=0.75$--$0.79$ on a 16-token smoke, but stock llama.cpp
verify with \texttt{-ub 1} \emph{slows} that \texttt{llama-cli} 12-slot run to
$0.80\times$ (baseline 0.30\,\tps{}, not the 0.38\,\tps{}
\texttt{llama-completion} greedy). Serial expert waves at 12 slots make
multi-token verify legal without $3\,n_{\mathrm{expert\_used}}{=}24$ resident
slots; batched verify (\texttt{-ub 8}) returns wall-clock to $\approx 1.0\times$.
Causal lag-2 prefetch recovers 0\% of the LRU--OPT gap on these traces. A local
\texttt{--moe-stream-umax} v1 ran on this box and \emph{lost} ($\alpha{=}0$,
$0.79\times$/$0.73\times$); we do not cite it as a HorizonSpec speedup.
Trace-oracle $U_{\max}$ (Table~\ref{tab:hs}) remains a simulation, not that
engine result.
\end{abstract}
...
novelty. Speculative decoding \citep{leviathan2023speculative} supplies lookahead
that greedy prefetch cannot. The systems question is whether the \emph{union}
of experts over a draft prefix fits in a RAM-sized slot budget---a
\emph{capacity law}, not a bandwidth slogan.
...
\item \textbf{Paired speculation:} high $\alpha$, negative wall-clock at
\texttt{-ub 1}, parity after serial-wave batched verify.
\item \textbf{Negatives:} $U(4)\le 16$ fails; causal prefetch recovery $W{=}0$;
8-slot HorizonSpec mean admitted length $T{=}1.003$.
\end{enumerate}
...
A small draft proposes tokens; the target verifies a prefix in parallel with a
sampling scheme that leaves the target distribution unchanged, giving
2$\times$--3$\times$ wall-time acceleration on T5-XXL
\citep{leviathan2023speculative}. Medusa and EAGLE change how drafts are
proposed, not the verify contract \citep{cai2024medusa,li2024eagle}. On an MoE
whose experts live on SSD, verify is cheap only if the \emph{union} of experts
over the prefix fits in the expert cache. That is the gap this paper measures.

\paragraph{Capacity law.}
Let $s$ be the number of expert slots per MoE layer that fit in RAM, $k$ the
draft length, and $U(k)$ the number of unique experts in a $k$-token window at
one layer. Greedy decode always loads $U(1)=\mathrm{top}\text{-}8=8$ experts per
layer. A $k$-token verify is one I/O generation only if $U(k)\le s$. If the
engine instead instantiates a worst-case graph of $k\times 8$ experts, extra
waves pay compute and I/O even when the realized union is small.
Admission-time $U_{\max}{=}s$ truncates the draft so the union fits. On traces
we simulate longest-prefix admission with \emph{target} routing (an upper
bound). A local engine v1 (\texttt{--moe-stream-umax}) truncated the
sampled+draft ubatch to $T{=}1$ (log: admitting $1/3$ and $1/5$, uniq${=}8$).
That layout is expected for greedy-only union. Draft $\alpha$ also collapsed to
0: row-0 logits disagreed with F10b (not ``T=1 dropped draft[0]''---the sampler
still compares draft[0] at $T{=}1$). Empty-wave \texttt{skip\_mul\_mat\_id} under
$\mathrm{plan\_n\_waves}{=}1$ is the leading unmeasured mechanism. The run is
not a bit-exact A/B, not F4, and not a tok/s win. The F4 table is still
per-layer-mean oracle routing.
...
\texttt{1248fd8fa}, \texttt{GGML\_CUDA=OFF}. Local patches on that commit:
serial expert waves when $n_{\mathrm{slots}}<3\,n_{\mathrm{expert\_used}}=24$
(top-8, not draft length), ordered-wave waits (an
OpenMP assert otherwise hangs), no next-wave pin when the cache is already
full, and optional \texttt{--moe-stream-umax} (measured lose; not in
Table~\ref{tab:spec}). Model: \texttt{Qwen3-235B-A22B-Q4\_K\_M} (3-way split, 133\,GB) and draft
\texttt{Qwen3-0.6B-Q8\_0} (768\,MB, dense) on ext4. Expert cache
...
layers), not a 4-token union and not engine $\min$ over 94 layers.
...
1.54 \emph{per layer}. The engine takes $\min$ admitted length over 94 layers
(\texttt{measurements/\_min\_layer\_T.json}): pooled engine $T{=}1.001$,
$p(T{\ge}2){=}0.12\%$, $\approx$43 overflow layers per window. A global
$U{\le}12$ gate therefore repeats $T{=}1$. Naive $k{=}4$ fits 1.2\%: stock
$k{=}4$ verify still wants $\sim$19 unique experts. That is why ``just set draft
$k{=}4$'' is the wrong lever, and why 8-slot speculation is a no-op on this
model independent of draft quality.

\section{Decode and speculation}
\label{sec:decode}

\paragraph{Greedy CPU decode}
...
Qwen3-0.6B, \texttt{--spec-type draft-simple}. On 30B (not SSD-bound at 4\,GiB
cache) $\alpha=0.857/0.833$ at $k{=}2/4$ and wall-clock $\approx 1.0\times$
(\texttt{measurements/spec30b\_verdict.json}, 37 gen tokens). On 235B, 16 eval tokens,
Fibonacci prompt: $\alpha=0.75$ ($k{=}2$, 9/12) and $0.79$ ($k{=}4$, 11/14).
Same-family drafting works. $n{=}16$ is a smoke, not a 200-prompt gate.

\paragraph{Wall-clock.}
llama.cpp with \texttt{-ub 1} serializes verify. At 12 slots, that
\emph{slows} 235B (Table~\ref{tab:spec}). Stock overlap waves require
$n_{\mathrm{slots}}\ge 3\,n_{\mathrm{expert\_used}}{=}24$ (Metal parking; the
factor is top-8, not draft length $k$). We run \emph{serial} waves
with $\mathrm{cap}=n_{\mathrm{slots}}$ when that inequality fails, wait if
wave custom-ops enter out of order, and do not pin the next wave's residents
when $n_{\mathrm{slots}}$ already equals plan capacity (that deadlock appeared
on the second 4-token ubatch). Prefill \texttt{-ub 4} then completes at
0.28 prompt \tps{} vs 0.16 at \texttt{-ub 1} on 16 tokens
(\texttt{llama-completion}, \texttt{-b 1 -ub 1} then \texttt{-b 8 -ub 4},
second arm warmer; not the \texttt{llama-cli} 28-token prompt 0.44\,\tps{}).

\begin{table}[t]\centering
\caption{235B speculation, 12-slot stream-direct, $n{=}16$, seed 1.
Eval tok/s from slot \texttt{print\_timing}
(\texttt{spec235b\_verdict.json}, \texttt{spec235b\_ub8/verdict.json}).
Two-decimal tok/s; eval-ms/tok \emph{speedup} vs this baseline is
$0.951\times$ / $0.977\times$ (values ${<}1$ mean slower).}
\label{tab:spec}
\small
\begin{tabular}{lcccc}
\toprule
arm & \texttt{-ub 1} eval \tps{} & \texttt{-ub 8} eval \tps{} & \texttt{-ub 8} ms/tok & $\alpha$ \\
\midrule
...
\begin{axis}[
  ...
  ylabel={eval speedup vs that arm's baseline},
  ymin=0.6, ymax=1.15,
  symbolic x coords={k2,k4},
  xtick=data,
  xticklabels={$k{=}2$,$k{=}4$},
  legend style={font=\small, at={(0.02,0.98)}, anchor=north west, draw=none},
  grid=major, grid style={gray!20},
...
\addplot[fill=wall!70] coordinates {(k2,0.80) (k4,0.77)};
\addlegendentry{\texttt{-ub 1}}
\addplot[fill=ok!80] coordinates {(k2,0.97) (k4,1.00)};
\addlegendentry{\texttt{-ub 8} serial waves}
\draw[dashed] (axis cs:k2,1) -- (axis cs:k4,1);
\end{axis}
\end{tikzpicture}
\caption{Stock \texttt{-ub 1} verify loses; serial-wave \texttt{-ub 8} returns
near parity (2-decimal tok/s; $k{=}4$ eval-ms/tok \emph{speedup} is
$0.977\times$, i.e.\ slightly slower). Neither bar is a HorizonSpec engine
speedup.}
\label{fig:spec}
\end{figure}

Batched verify \textbf{stops the $0.80\times$ tax}. It does \textbf{not} deliver
the Table~\ref{tab:hs} \emph{per-layer-mean} $T\approx 1.54$ projection (engine
$\min$ $T$ at 12 slots is 1.001): the graph is still sized for
worst-case unique experts, Table~\ref{tab:spec} has no $U_{\max}$ (a later
engine v1 admitted $T{=}1$ and lost $\alpha$), and $n{=}16$ is a smoke. Skipping compiled empty-wave expert GEMMs
(\texttt{measurements/spec235b\_skip/verdict.json}) leaves $k{=}2$ eval at
3251\,ms/tok vs 3249 without the skip; $k{=}4$ is 3039 vs 3163 on a later,
warmer arm---we do not claim a skip speedup. Peak RSS 16.7--17.6\,GiB.
\texttt{llama-cli -b 1} cannot greedy-decode this model (assert
$n_{\mathrm{tokens}}\le n_{\mathrm{batch}}$ after skipping the fused-GDN
probe); the working invocation is \texttt{-b 512 -ub \{1,8\}}.
...
ceiling is what a draft can supply only if verify can hold the union---the
case for admission-time $U_{\max}$, not for lag-1 prefetch.
...
All 235B decode numbers are one prompt length and one seed unless stated. We
did not run 200-prompt $\alpha$. Table~\ref{tab:hs} is per-layer-mean oracle
routing, not engine tok/s and not $\min$ over layers (that min is $T{=}1.001$
at 12 slots $k{=}2$; \texttt{\_min\_layer\_T.json}). A \texttt{--moe-stream-umax} smoke admitted $T{=}1$ of a
sampled+draft ubatch ($\alpha{=}0$, $0.79\times$/$0.73\times$;
\texttt{spec235b\_umax/verdict.json}); we do not treat prefix FFN as
bit-exact---that was unmeasured. 12-slot serial waves are CPU-only; Metal
still wants 24 slots. We did not load 235B on the 8\,GB GPU. The 0.44\,\tps{}
GPU-split figure is excluded. Empty-wave GEMM skip is a local tree change;
Table~\ref{tab:spec} is the pre-skip \texttt{-ub 8} pair. The skip re-time does
not move $k{=}2$ eval ms/tok.
...
parallel \citep{leviathan2023speculative,cai2024medusa,li2024eagle}. SiDA-MoE
predicts experts for throughput \citep{du2024sidamoe}. Concurrent MoE
speculative-decoding papers also name the verify \emph{union}: MoE-Spec loads a
fixed expert budget $B$ and drops the long tail
\citep{mcdanel2026moespec}; AcceptMoE restricts eligible experts and states
that this changes the model distribution \citep{liang2026acceptmoe}; EcoSpec
selects draft paths by predicted expert cost without changing the target
verifier, on Qwen3-235B-A22B with 8 H200 GPUs \citep{xie2026ecospec}; EVICT
truncates the draft tree before verify (same 235B model, tensor-parallel 8 on
A100) \citep{pan2026evict}. Those are GPU/HBM or expert-\emph{drop} levers. A
12-slot CPU stream cannot use a global $U{\le}12$ gate (engine $\min$ $T{=}1.001$);
the remaining exact-routing lever is prefix FFN plus overflow waves.
The closest measurement ancestors remain Mixtral offload (2--3\,\tps{} on
8$\times$7B, not 235B) and MoE-Infinity (expert-level traces on a GPU that holds
the working set). Training-time router locality on 137M models is a companion
negative result \citep{suram2026cacheable} and is not this paper's tok/s
ladder. We contribute the 235B top-8 union law on a RAM slab that does
\emph{not} hold the table, and the speculation negative-then-parity pair under
that law.
...
stream. Speculation has the right $\alpha$ and the wrong verify granularity
until multi-token ubatch is legal; serial waves restore parity, not a
$1.5\times$ win. Per-layer-mean oracle $U_{\max}$ still says 54\% of 12-slot
$k{=}2$ windows fit ($T{=}1.54$); engine $\min$ over 94 layers is $T{=}1.001$
($p(T{\ge}2){=}0.12\%$). The engine v1 admitted one ubatch row and lost
$\alpha$. The remaining lever is: never skip row-0 FFN, always FFN the first
draft row (serial waves if $U(2){>}12$), and do not set global
\texttt{skip\_mul\_mat\_id} under umax---not a global $U{\le}12$ gate. The
publishable claims are
the traces, the union law, the mmap/\texttt{O\_DIRECT} ladder, and the
speculation negative-then-parity pair---not umax tok/s. We post this as a
measurement preprint.
It is not a TMLR resubmission of the companion training-time locality paper
\citep{suram2026cacheable}.
...
\texttt{1248fd8fa} plus local serial-wave patches. CPU-only; GPU idle.
...
that differ in binary, prompt, \texttt{n\_ubatch}, or cache size.}
\label{tab:prov}
\small
...
spec \texttt{-ub 1} $k{=}2$ & 0.24\,\tps{}, $0.80\times$ & \texttt{spec235b\_verdict.json} \\
spec \texttt{-ub 8} $k{=}4$ & 0.32\,\tps{}, $1.00\times$ tok/s ($0.977\times$ ms/tok) & \texttt{spec235b\_ub8/verdict.json} \\
empty-wave skip $k{=}2$ & 3251 vs 3249\,ms/tok & \texttt{spec235b\_skip/verdict.json} \\
umax v1 $k{=}2$ & 0.26\,\tps{}, $\alpha{=}0$, $0.79\times$ & \texttt{spec235b\_umax/verdict.json} \\
lag-2 @ cap 16 & 0\% of LRU--W8 gap & \texttt{g2a\_235b.json} \\
30B spec $\alpha$ & 0.86 / 0.83 & \texttt{spec30b\_verdict.json} \\
\bottomrule
\end{tabular}
\end{table}
